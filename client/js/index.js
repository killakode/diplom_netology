document.addEventListener("DOMContentLoaded", () => {
    let WeekElements = document.querySelectorAll('.page-nav__day-week');
    let dayElements = document.querySelectorAll('.page-nav__day-number');
    let pageNavDays = document.querySelectorAll('.page-nav__day');
  
    dayElements.forEach(function (dayNumberElement, index) {
      let day = new Date();
      let navDay = dayNumberElement.parentNode;
      day.setDate(day.getDate() + index);
      dayNumberElement.textContent = day.getDate();
      WeekElements[index].textContent = weekDays(day);


      if (WeekElements[index].textContent === 'Сб' || WeekElements[index].textContent === 'Вс') {
